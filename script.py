...
USERNAME = os.environ.get("PUNCH_USERNAME", "varun.singh@telesys.com")
PASSWORD = os.environ.get("PUNCH_PASSWORD", "telesys")
...
def run():
    with sync_playwright() as p:
        ...
        page.goto("https://telesyssoftware.securtime.adp.com/login?redirectUrl=%2Fwelcome")

        page.fill('input[type="email"]', USERNAME)
        page.fill('input[type="password"]', PASSWORD)
        page.click('button[type="submit"]:has-text("Sign In")')

        page.wait_for_selector('h2:has-text("Welcome!")', timeout=30000)
        ...
        if ACTION == "in":
            ...
            # Wait for the login screen to reappear and log in again
            page.wait_for_selector('input[type="email"]', timeout=30000)
            page.fill('input[type="email"]', USERNAME)
            page.fill('input[type="password"]', PASSWORD)
            page.click('button[type="submit"]:has-text("Sign In")')

            # Wait for the welcome text again after login
            page.wait_for_selector('h2:has-text("Welcome!")', timeout=30000)
